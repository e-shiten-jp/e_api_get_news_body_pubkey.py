...
#
# 必要な設定項目
# ニュースID（レコード毎にユニーク）: P_ID    ニュースヘッダーで取得したp_IDを指定する。
#
# 利用方法: 
...
# コマンド用パラメーター -------------------    
P_ID = ''
                # 本文を取得したいニュースID（レコード毎にユニーク）。
                # ニュースヘッダーで取得したp_IDを指定する。例:20230825180000_OGL9800
                # ニュースヘッダー取得で返される第一項目
FNAME_OUTPUT = 'news_body_' + P_ID + '.csv'   # 書き込むファイル名。カレントディレクトリに上書きモードでファイルが作成される。
...
if __name__ == "__main__":
    ...
    dic_req_item = {
        ...
        'sCLMID':               'CLMMfdsGetNewsBody',   # ニュースボディー問合取得
        'p_ID':                 P_ID,                   # カニュースＩＤ	情報（レコード）識別用ユニーク値
                                                        # ニュースヘッダー取得で返された「p_ID	ニュースＩＤ	情報（レコード）識別用ユニーク値」
        'sJsonOfmt':            str_sJsonOfmt           # 表示形式（サポートへの問い合わせでは'5'を指定指定した送信電文と受信電文で。）
    }

    # 'CLMMfdsGetNewsBody'は、仮想URL:'sUrlMaster'
    ...
    if dic_return is None:
        ...
        if dic_return.get('p_errno') != '-2' and dic_return.get('p_errno') != '2':
            ...
            if my_list_return is not None:
                # 出力ファイルにタイトル行を書き込む。
                func_write_news_body_title(FNAME_OUTPUT)
                ...
                func_write_news_body_data(FNAME_OUTPUT, my_list_return)
            else:
